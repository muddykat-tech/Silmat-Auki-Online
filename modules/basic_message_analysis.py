...
def generate_message_html(analyzed_content, original_message):
    ...
    eyeData = EyeData("data", original_message)
    trigram_message = eyeData.get_raw_trigram_data()
    ascii_message = eyeData.get_trigram_values()
...
def basic_analysis_page():
    ...
    if request.method == 'POST':
        selected_message = request.json.get('message')
...
class EyeData:
    ...
    def calculate_trigrams_from_raw(self):
        ...
        for trigram_line in raw_trigram_data:
            ...
            raw_line_data = [Trigram(trigram) if i % 2 == 0 else Trigram(trigram[::-1]) for i, trigram in enumerate(trigrams)]
    ...
    def get_raw_trigram_data(self):
        return '\n'.join([','.join([trigram.get_trigram_string(self.selected_mode) for trigram in line]) for line in self.trigram_line_data])

    def get_trigram_values(self):
        return '\n'.join([','.join([trigram.get_trigram_value(self.selected_mode) for trigram in line]) for line in self.trigram_line_data])
